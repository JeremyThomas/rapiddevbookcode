...
namespace AW.Winforms.Helpers
{
  ...
  public class WindowSettings
  {
    ...
    ///   The splitters to restore. You can change some entries
    ///   to null if you no longer use that position in the list.
    /// </param>
    ...
    private static int GetSplitterDistance(SplitContainer splitter)
    {
      var splitterSize =
        splitter.Orientation == Orientation.Vertical
          ? ((Panel) splitter.Panel1).Width
          : ((Panel) splitter.Panel1).Height;
      return splitterSize;
    ...
    private void RecordSplitters(IList<SplitContainer> splitters)
    {
      ...
      for (var i = 0; i < splitters.Count; i++)
      {
        ...
        {
          SplitterDistances[i] = GetSplitterDistance(splitContainer);
