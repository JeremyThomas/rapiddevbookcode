...
public partial class Controls_ListSalesOrderDetailInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.SalesOrderDetailEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("SalesOrderDetail.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		SalesOrderDetailEntity selectedEntity = (SalesOrderDetailEntity)_SalesOrderDetailDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&SalesOrderDetailID={0}", selectedEntity.SalesOrderDetailID);
		pkFieldsAndValues.AppendFormat("&SalesOrderID={0}", selectedEntity.SalesOrderID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.SalesOrderDetailEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.SalesOrderDetailEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
			...
			case "SalesOrderHeaderEntity":
				switch(name)
				{
					case "SalesOrderDetails":
						...
						dsFilter.AddWithAnd(SalesOrderDetailFields.SalesOrderID == ((SalesOrderHeaderEntity)containingEntity).SalesOrderID);
						_SalesOrderDetailDS.FilterToUse = dsFilter;
				...
				break;
			case "SpecialOfferProductEntity":
				switch(name)
				{
					case "SalesOrderDetails":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(SalesOrderDetailFields.ProductID == ((SpecialOfferProductEntity)containingEntity).ProductID);
						dsFilter.AddWithAnd(SalesOrderDetailFields.SpecialOfferID == ((SpecialOfferProductEntity)containingEntity).SpecialOfferID);
						_SalesOrderDetailDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&ProductID=" + 
								((SpecialOfferProductEntity)containingEntity).ProductID +
								"&20FkField=ProductID";
						break;
					default:
						break;
	...
	{
		set { _SalesOrderDetailDS.FilterToUse = value; }
	}
