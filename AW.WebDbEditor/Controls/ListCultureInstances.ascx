...
<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ListCultureInstances.ascx.cs" Inherits="Controls_ListCultureInstances" %>
...
<asp:GridView ID="theGrid" runat="server" SkinID="gridviewSkin" AutoGenerateColumns="False" DataKeyNames="CultureID" 
					DataSourceID="_CultureDS" OnRowCommand="theGrid_RowCommand">
	<Columns>
		...
		<asp:TemplateField ShowHeader="False" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" HeaderStyle-CssClass="tableheaders" HeaderStyle-HorizontalAlign="Left">
			<ItemTemplate>
				<asp:Button ID="btnDeleteExisting" runat="server" CausesValidation="false" CommandArgument='<%# DataBinder.Eval(Container,"RowIndex") %>' CommandName="Delete" Text="Delete" SkinID="ButtonSkin" OnClientClick='<%# "return confirm(\"Are you sure you want to delete this Culture instance with PK values: "  + "CultureID: " + Eval("CultureID") + " ?\");" %>'/>
			</ItemTemplate>
		</asp:TemplateField>
