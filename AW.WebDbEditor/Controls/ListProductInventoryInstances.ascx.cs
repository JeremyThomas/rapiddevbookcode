...
public partial class Controls_ListProductInventoryInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.ProductInventoryEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("ProductInventory.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		ProductInventoryEntity selectedEntity = (ProductInventoryEntity)_ProductInventoryDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&LocationID={0}", selectedEntity.LocationID);
		pkFieldsAndValues.AppendFormat("&ProductID={0}", selectedEntity.ProductID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.ProductInventoryEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.ProductInventoryEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "LocationEntity":
				switch(name)
				{
					case "ProductInventories":
						...
						dsFilter.AddWithAnd(ProductInventoryFields.LocationID == ((LocationEntity)containingEntity).LocationID);
						_ProductInventoryDS.FilterToUse = dsFilter;
			...
			case "ProductEntity":
				switch(name)
				{
					case "ProductInventories":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(ProductInventoryFields.ProductID == ((ProductEntity)containingEntity).ProductID);
						_ProductInventoryDS.FilterToUse = dsFilter;
	...
	{
		set { _ProductInventoryDS.FilterToUse = value; }
	}
