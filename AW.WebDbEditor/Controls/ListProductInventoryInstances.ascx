...
<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ListProductInventoryInstances.ascx.cs" Inherits="Controls_ListProductInventoryInstances" %>
...
<llblgenpro:LLBLGenProDataSource ID="_ProductInventoryDS" runat="server" DataContainerType="EntityCollection" EntityCollectionTypeName="AW.Data.CollectionClasses.ProductInventoryCollection, AW.Data" CacheLocation="Session" EnablePaging="True" />
<asp:GridView ID="theGrid" runat="server" SkinID="gridviewSkin" AutoGenerateColumns="False" DataKeyNames="LocationID, ProductID" 
					DataSourceID="_ProductInventoryDS" OnRowCommand="theGrid_RowCommand">
	<Columns>
		...
		<asp:TemplateField ShowHeader="False" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" HeaderStyle-CssClass="tableheaders" HeaderStyle-HorizontalAlign="Left">
			<ItemTemplate>
				<asp:Button ID="btnDeleteExisting" runat="server" CausesValidation="false" CommandArgument='<%# DataBinder.Eval(Container,"RowIndex") %>' CommandName="Delete" Text="Delete" SkinID="ButtonSkin" OnClientClick='<%# "return confirm(\"Are you sure you want to delete this ProductInventory instance with PK values: "  + "LocationID: " + Eval("LocationID") + ", " + "ProductID: " + Eval("ProductID") + " ?\");" %>'/>
			</ItemTemplate>
		</asp:TemplateField>
		<asp:TemplateField HeaderText="Bin" SortExpression="Bin"  HeaderStyle-HorizontalAlign="Left" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" >
			<ItemTemplate>
				<asp:Label ID="lblBin" runat="server" Text='<%# Bind("Bin") %>'/>
			</ItemTemplate>
		</asp:TemplateField>
		...
		<asp:TemplateField HeaderText="Shelf" SortExpression="Shelf"  HeaderStyle-HorizontalAlign="Left" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" >
			<ItemTemplate>
				<asp:Label ID="lblShelf" runat="server" Text='<%# Bind("Shelf") %>'/>
			</ItemTemplate>
		</asp:TemplateField>
