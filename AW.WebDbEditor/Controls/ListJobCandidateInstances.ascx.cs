...
public partial class Controls_ListJobCandidateInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.JobCandidateEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("JobCandidate.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		JobCandidateEntity selectedEntity = (JobCandidateEntity)_JobCandidateDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&JobCandidateID={0}", selectedEntity.JobCandidateID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.JobCandidateEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.JobCandidateEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
...
IPredicateExpression dsFilter;
			...
			case "EmployeeEntity":
				switch(name)
				{
					case "JobCandidates":
						...
						dsFilter.AddWithAnd(JobCandidateFields.EmployeeID == ((EmployeeEntity)containingEntity).EmployeeID);
						_JobCandidateDS.FilterToUse = dsFilter;
	...
	{
		set { _JobCandidateDS.FilterToUse = value; }
	}
