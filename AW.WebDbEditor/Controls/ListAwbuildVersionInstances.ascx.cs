...
public partial class Controls_ListAwbuildVersionInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.AwbuildVersionEntity + additionalFiltersAsString);
	}
	...
	/// </summary>
		...
		// export to excel
				GridViewExportUtils.Export("AwbuildVersion.xls", theGrid);
		}
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		AwbuildVersionEntity selectedEntity = (AwbuildVersionEntity)_AwbuildVersionDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&SystemInformationID={0}", selectedEntity.SystemInformationID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.AwbuildVersionEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.AwbuildVersionEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
...
IPredicateExpression dsFilter;
	...
	{
		set { _AwbuildVersionDS.FilterToUse = value; }
	}
