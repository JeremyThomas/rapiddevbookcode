...
public partial class Controls_ListEmployeeDepartmentHistoryInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.EmployeeDepartmentHistoryEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("EmployeeDepartmentHistory.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		EmployeeDepartmentHistoryEntity selectedEntity = (EmployeeDepartmentHistoryEntity)_EmployeeDepartmentHistoryDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&DepartmentID={0}", selectedEntity.DepartmentID);
		pkFieldsAndValues.AppendFormat("&EmployeeID={0}", selectedEntity.EmployeeID);
		pkFieldsAndValues.AppendFormat("&ShiftID={0}", selectedEntity.ShiftID);
		pkFieldsAndValues.AppendFormat("&StartDate={0}", selectedEntity.StartDate);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.EmployeeDepartmentHistoryEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.EmployeeDepartmentHistoryEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "DepartmentEntity":
				switch(name)
				{
					case "EmployeeDepartmentHistories":
						...
						dsFilter.AddWithAnd(EmployeeDepartmentHistoryFields.DepartmentID == ((DepartmentEntity)containingEntity).DepartmentID);
						_EmployeeDepartmentHistoryDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&DepartmentID=" + 
								((DepartmentEntity)containingEntity).DepartmentID +
								"&20FkField=DepartmentID";
						break;
					default:
						break;
				}
			...
			case "EmployeeEntity":
				switch(name)
				{
					case "EmployeeDepartmentHistories":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(EmployeeDepartmentHistoryFields.EmployeeID == ((EmployeeEntity)containingEntity).EmployeeID);
						_EmployeeDepartmentHistoryDS.FilterToUse = dsFilter;
			...
			case "ShiftEntity":
				switch(name)
				{
					case "EmployeeDepartmentHistories":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(EmployeeDepartmentHistoryFields.ShiftID == ((ShiftEntity)containingEntity).ShiftID);
						_EmployeeDepartmentHistoryDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&ShiftID=" + 
								((ShiftEntity)containingEntity).ShiftID +
								"&20FkField=ShiftID";
						break;
					default:
						break;
	...
	{
		set { _EmployeeDepartmentHistoryDS.FilterToUse = value; }
	}
