...
<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ListEmployeeDepartmentHistoryInstances.ascx.cs" Inherits="Controls_ListEmployeeDepartmentHistoryInstances" %>
...
<asp:GridView ID="theGrid" runat="server" SkinID="gridviewSkin" AutoGenerateColumns="False" DataKeyNames="DepartmentID, EmployeeID, ShiftID, StartDate" 
					DataSourceID="_EmployeeDepartmentHistoryDS" OnRowCommand="theGrid_RowCommand">
	<Columns>
		...
		<asp:TemplateField ShowHeader="False" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" HeaderStyle-CssClass="tableheaders" HeaderStyle-HorizontalAlign="Left">
			<ItemTemplate>
				<asp:Button ID="btnDeleteExisting" runat="server" CausesValidation="false" CommandArgument='<%# DataBinder.Eval(Container,"RowIndex") %>' CommandName="Delete" Text="Delete" SkinID="ButtonSkin" OnClientClick='<%# "return confirm(\"Are you sure you want to delete this EmployeeDepartmentHistory instance with PK values: "  + "DepartmentID: " + Eval("DepartmentID") + ", " + "EmployeeID: " + Eval("EmployeeID") + ", " + "ShiftID: " + Eval("ShiftID") + ", " + "StartDate: " + Eval("StartDate") + " ?\");" %>'/>
			</ItemTemplate>
		</asp:TemplateField>
		<asp:TemplateField HeaderText="DepartmentID" SortExpression="DepartmentID"  HeaderStyle-HorizontalAlign="Left" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" >
			<ItemTemplate>
				<asp:HyperLink ID="lnkDepartmentID" runat="server" NavigateUrl='<%# "~/ViewExisting.aspx?EntityType=" + ((int)EntityType.DepartmentEntity).ToString() + "&DepartmentID=" + Eval("DepartmentID")%>' Text='<%# Bind("DepartmentID") %>'/>
			</ItemTemplate>
		</asp:TemplateField>
		...
		<asp:TemplateField HeaderText="ShiftID" SortExpression="ShiftID"  HeaderStyle-HorizontalAlign="Left" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" >
			<ItemTemplate>
				<asp:HyperLink ID="lnkShiftID" runat="server" NavigateUrl='<%# "~/ViewExisting.aspx?EntityType=" + ((int)EntityType.ShiftEntity).ToString() + "&ShiftID=" + Eval("ShiftID")%>' Text='<%# Bind("ShiftID") %>'/>
			</ItemTemplate>
		</asp:TemplateField>
