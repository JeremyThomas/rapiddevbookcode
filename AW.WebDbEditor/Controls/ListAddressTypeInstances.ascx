...
<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ListAddressTypeInstances.ascx.cs" Inherits="Controls_ListAddressTypeInstances" %>
...
<llblgenpro:LLBLGenProDataSource ID="_AddressTypeDS" runat="server" DataContainerType="EntityCollection" EntityCollectionTypeName="AW.Data.CollectionClasses.AddressTypeCollection, AW.Data" CacheLocation="Session" EnablePaging="True" />
<asp:GridView ID="theGrid" runat="server" SkinID="gridviewSkin" AutoGenerateColumns="False" DataKeyNames="AddressTypeID" 
					DataSourceID="_AddressTypeDS" OnRowCommand="theGrid_RowCommand">
	<Columns>
		...
		<asp:TemplateField ShowHeader="False" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" HeaderStyle-CssClass="tableheaders" HeaderStyle-HorizontalAlign="Left">
			<ItemTemplate>
				<asp:Button ID="btnDeleteExisting" runat="server" CausesValidation="false" CommandArgument='<%# DataBinder.Eval(Container,"RowIndex") %>' CommandName="Delete" Text="Delete" SkinID="ButtonSkin" OnClientClick='<%# "return confirm(\"Are you sure you want to delete this AddressType instance with PK values: "  + "AddressTypeID: " + Eval("AddressTypeID") + " ?\");" %>'/>
			</ItemTemplate>
		</asp:TemplateField>
		<asp:TemplateField HeaderText="AddressTypeID" SortExpression="AddressTypeID" ItemStyle-HorizontalAlign="Right" HeaderStyle-HorizontalAlign="Right" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" >
			<ItemTemplate>
				<asp:Label ID="lblAddressTypeID" runat="server" Text='<%# Bind("AddressTypeID") %>'/>
			</ItemTemplate>
		</asp:TemplateField>
