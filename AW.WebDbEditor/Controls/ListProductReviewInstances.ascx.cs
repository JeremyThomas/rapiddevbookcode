...
public partial class Controls_ListProductReviewInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.ProductReviewEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("ProductReview.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		ProductReviewEntity selectedEntity = (ProductReviewEntity)_ProductReviewDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&ProductReviewID={0}", selectedEntity.ProductReviewID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.ProductReviewEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.ProductReviewEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
...
IPredicateExpression dsFilter;
			...
			case "ProductEntity":
				switch(name)
				{
					case "ProductReviews":
						...
						dsFilter.AddWithAnd(ProductReviewFields.ProductID == ((ProductEntity)containingEntity).ProductID);
						_ProductReviewDS.FilterToUse = dsFilter;
	...
	{
		set { _ProductReviewDS.FilterToUse = value; }
	}
