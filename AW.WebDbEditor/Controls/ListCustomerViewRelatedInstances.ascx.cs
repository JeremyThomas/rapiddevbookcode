...
public partial class Controls_ListCustomerViewRelatedInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.CustomerViewRelatedEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("CustomerViewRelated.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		CustomerViewRelatedEntity selectedEntity = (CustomerViewRelatedEntity)_CustomerViewRelatedDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&CustomerId={0}", selectedEntity.CustomerId);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.CustomerViewRelatedEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.CustomerViewRelatedEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
...
IPredicateExpression dsFilter;
	...
	{
		set { _CustomerViewRelatedDS.FilterToUse = value; }
	}
