...
public partial class Controls_ListWorkOrderInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.WorkOrderEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("WorkOrder.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		WorkOrderEntity selectedEntity = (WorkOrderEntity)_WorkOrderDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&WorkOrderID={0}", selectedEntity.WorkOrderID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.WorkOrderEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.WorkOrderEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
			...
			case "ProductEntity":
				switch(name)
				{
					case "WorkOrders":
						...
						dsFilter.AddWithAnd(WorkOrderFields.ProductID == ((ProductEntity)containingEntity).ProductID);
						_WorkOrderDS.FilterToUse = dsFilter;
				...
				break;
			case "ScrapReasonEntity":
				switch(name)
				{
					case "WorkOrders":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(WorkOrderFields.ScrapReasonID == ((ScrapReasonEntity)containingEntity).ScrapReasonID);
						_WorkOrderDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&ScrapReasonID=" + 
								((ScrapReasonEntity)containingEntity).ScrapReasonID +
								"&20FkField=ScrapReasonID";
						break;
					default:
						break;
				}
				break;
			case "WorkOrderRoutingEntity":
				switch(name)
				{
					default:
						break;
	...
	{
		set { _WorkOrderDS.FilterToUse = value; }
	}
