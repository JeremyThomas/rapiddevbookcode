...
public partial class Controls_ListCurrencyRateInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.CurrencyRateEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("CurrencyRate.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		CurrencyRateEntity selectedEntity = (CurrencyRateEntity)_CurrencyRateDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&CurrencyRateID={0}", selectedEntity.CurrencyRateID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.CurrencyRateEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.CurrencyRateEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
			...
			case "CurrencyEntity":
				switch(name)
				{
					case "CurrencyRates":
						...
						dsFilter.AddWithAnd(CurrencyRateFields.FromCurrencyCode == ((CurrencyEntity)containingEntity).CurrencyCode);
						_CurrencyRateDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&FromCurrencyCode=" + 
								((CurrencyEntity)containingEntity).CurrencyCode +
								"&20FkField=FromCurrencyCode";
						break;
					case "CurrencyRates_":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(CurrencyRateFields.ToCurrencyCode == ((CurrencyEntity)containingEntity).CurrencyCode);
						_CurrencyRateDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&ToCurrencyCode=" + 
								((CurrencyEntity)containingEntity).CurrencyCode +
								"&20FkField=ToCurrencyCode";
						break;
					default:
						break;
				}
	...
	{
		set { _CurrencyRateDS.FilterToUse = value; }
	}
