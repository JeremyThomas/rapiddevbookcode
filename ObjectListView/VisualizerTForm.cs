...
namespace JesseJohnston
{
	internal partial class VisualizerTForm<T> : Form
	{
		...
		private void ShowListItemProperties()
		{
			...
			this.toolTip.SetToolTip(this.labelListItemType, listItemType.FullName);
			...
			List<string> notifyingProps = GetPropertyChangedEvents();
		...
		{
			this.listItemTypeName = typeof(T).Name;
		...
		private List<string> GetPropertyChangedEvents()
		{
			...
			{
				EventInfo propChangedEvent = listItemType.GetEvent(prop.Name + "Changed");
