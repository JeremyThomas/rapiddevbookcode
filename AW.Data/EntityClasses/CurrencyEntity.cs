...
namespace AW.Data.EntityClasses
{
		...
		// __LLBLGENPRO_USER_CODE_REGION_END	
	{
		...
		private AW.Data.CollectionClasses.CountryRegionCollection _countryRegions;
		private bool	_alwaysFetchCountryRegions, _alreadyFetchedCountryRegions;
		...
		public CurrencyEntity(System.String currencyCode, IPrefetchPath prefetchPathToUse):base("CurrencyEntity")
		{
		...
		protected CurrencyEntity(SerializationInfo info, StreamingContext context) : base(info, context)
		{
			...
			_countryRegions = (AW.Data.CollectionClasses.CountryRegionCollection)info.GetValue("_countryRegions", typeof(AW.Data.CollectionClasses.CountryRegionCollection));
			_alwaysFetchCountryRegions = info.GetBoolean("_alwaysFetchCountryRegions");
			_alreadyFetchedCountryRegions = info.GetBoolean("_alreadyFetchedCountryRegions");
		...
		protected override void PerformPostReadXmlFixups()
		{
			...
			_alreadyFetchedCountryRegions = (_countryRegions.Count > 0);
		}
		...
		protected override void GetObjectData(SerializationInfo info, StreamingContext context)
		{
			...
			info.AddValue("_countryRegions", (!this.MarkedForDeletion?_countryRegions:null));
			info.AddValue("_alwaysFetchCountryRegions", _alwaysFetchCountryRegions);
			info.AddValue("_alreadyFetchedCountryRegions", _alreadyFetchedCountryRegions);
		...
		protected override void SetRelatedEntityProperty(string propertyName, IEntityCore entity)
		{
			switch(propertyName)
			{
				...
				case "CurrencyRates_":
					...
					break;
				case "CountryRegions":
					_alreadyFetchedCountryRegions = true;
					if(entity!=null)
					{
						this.CountryRegions.Add((CountryRegionEntity)entity);
					}
					break;
				default:
		...
		public AW.Data.CollectionClasses.CountryRegionCollection GetMultiCountryRegions(bool forceFetch)
		{
			return GetMultiCountryRegions(forceFetch, _countryRegions.EntityFactoryToUse);
		}
		...
		/// <returns>Filled collection with all related entities of the type constructed by the passed in entity factory</returns>
		public AW.Data.CollectionClasses.CountryRegionCollection GetMultiCountryRegions(bool forceFetch, IEntityFactory entityFactoryToUse)
		{
 			if( ( !_alreadyFetchedCountryRegions || forceFetch || _alwaysFetchCountryRegions) && !this.IsSerializing && !this.IsDeserializing && !this.InDesignMode)
			{
				AddToTransactionIfNecessary(_countryRegions);
				...
				_countryRegions.SuppressClearInGetMulti=!forceFetch;
				_countryRegions.EntityFactoryToUse = entityFactoryToUse;
				_countryRegions.GetMulti(filter, GetRelationsForField("CountryRegions"));
				_countryRegions.SuppressClearInGetMulti=false;
				_alreadyFetchedCountryRegions = true;
			}
			return _countryRegions;
		}

		/// <summary> Sets the collection parameters for the collection for 'CountryRegions'. These settings will be taken into account
		/// when the property CountryRegions is requested or GetMultiCountryRegions is called.</summary>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return. When set to 0, this parameter is ignored</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified (null), no sorting is applied.</param>
		public virtual void SetCollectionParametersCountryRegions(long maxNumberOfItemsToReturn, ISortExpression sortClauses)
		{
			_countryRegions.SortClauses=sortClauses;
			_countryRegions.MaxNumberOfItemsToReturn=maxNumberOfItemsToReturn;
		}
		...
		protected override Dictionary<string, object> GetRelatedData()
		{
			...
			toReturn.Add("CountryRegions", _countryRegions);
			return toReturn;
		}
		...
		private void InitClassMembers()
		{
			...
			_countryRegions = new AW.Data.CollectionClasses.CountryRegionCollection();
		...
		/// <returns>Ready to use IPrefetchPathElement implementation.</returns>
		public static IPrefetchPathElement PrefetchPathCountryRegions
		{
			get
			{
				...
				return new PrefetchPathElement(new AW.Data.CollectionClasses.CountryRegionCollection(), intermediateRelation,	(int)AW.Data.EntityType.CurrencyEntity, (int)AW.Data.EntityType.CountryRegionEntity, 0, null, null, GetRelationsForField("CountryRegions"), "CountryRegions", SD.LLBLGen.Pro.ORMSupportClasses.RelationType.ManyToMany);
			}
		}
		...
		/// MS_Description: Currency name.<br/>Currency name.</summary>
		...
		/// </summary>
		/// <remarks>This property is added for databinding conveniance, however it is recommeded to use the method 'GetMultiCountryRegions()', because 
		/// this property is rather expensive and a method tells the user to cache the result when it has to be used more than once in the same scope.</remarks>
		public virtual AW.Data.CollectionClasses.CountryRegionCollection CountryRegions
		{
			get { return GetMultiCountryRegions(false); }
		}

		/// <summary> Gets / sets the lazy loading flag for CountryRegions. When set to true, CountryRegions is always refetched from the 
		/// persistent storage. When set to false, the data is only fetched the first time CountryRegions is accessed. You can always execute a forced fetch by calling GetMultiCountryRegions(true).</summary>
		[Browsable(false)]
		public bool AlwaysFetchCountryRegions
		{
			get	{ return _alwaysFetchCountryRegions; }
			set	{ _alwaysFetchCountryRegions = value; }
		}
				
		/// <summary>Gets / Sets the lazy loading flag if the property CountryRegions already has been fetched. Setting this property to false when CountryRegions has been fetched
		/// will clear the CountryRegions collection well. Setting this property to true while CountryRegions hasn't been fetched disables lazy loading for CountryRegions</summary>
		[Browsable(false)]
		public bool AlreadyFetchedCountryRegions
		{
			get { return _alreadyFetchedCountryRegions;}
			set 
			{
				if(_alreadyFetchedCountryRegions && !value && (_countryRegions != null))
				{
					_countryRegions.Clear();
				}
				_alreadyFetchedCountryRegions = value;
			}
		}
