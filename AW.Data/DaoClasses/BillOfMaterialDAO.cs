...
namespace AW.Data.DaoClasses
{
	...
	/// <summary>General DAO class for the BillOfMaterial Entity. It will perform database oriented actions for a entity of type 'BillOfMaterialEntity'.</summary>
	public partial class BillOfMaterialDAO : CommonDaoBase
	{
		/// <summary>CTor</summary>
		public BillOfMaterialDAO() : base(InheritanceHierarchyType.None, "BillOfMaterialEntity", new BillOfMaterialEntityFactory())
		{
		}



		/// <summary>Retrieves in the calling BillOfMaterialCollection object all BillOfMaterialEntity objects which have data in common with the specified related Entities. If one is omitted, that entity is not used as a filter. </summary>
		...
		public bool GetMulti(ITransaction containingTransaction, IEntityCollection collectionToFill, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IEntityFactory entityFactoryToUse, IPredicateExpression filter, IEntity productComponentInstance, IEntity productAssemblyInstance, IEntity unitMeasureInstance, int pageNumber, int pageSize)
		{
			...
			IPredicateExpression selectFilter = CreateFilterUsingForeignKeys(productComponentInstance, productAssemblyInstance, unitMeasureInstance, fieldsToReturn);
			if(filter!=null)
			{
		...
		public int DeleteMulti(ITransaction containingTransaction, IEntity productComponentInstance, IEntity productAssemblyInstance, IEntity unitMeasureInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(AW.Data.EntityType.BillOfMaterialEntity);
			IPredicateExpression deleteFilter = CreateFilterUsingForeignKeys(productComponentInstance, productAssemblyInstance, unitMeasureInstance, fields);
		...
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(IEntity entityWithNewValues, ITransaction containingTransaction, IEntity productComponentInstance, IEntity productAssemblyInstance, IEntity unitMeasureInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(AW.Data.EntityType.BillOfMaterialEntity);
			IPredicateExpression updateFilter = CreateFilterUsingForeignKeys(productComponentInstance, productAssemblyInstance, unitMeasureInstance, fields);
		...
		private IPredicateExpression CreateFilterUsingForeignKeys(IEntity productComponentInstance, IEntity productAssemblyInstance, IEntity unitMeasureInstance, IEntityFields fieldsToReturn)
		{
			...
			{
				selectFilter.Add(new FieldCompareValuePredicate(fieldsToReturn[(int)BillOfMaterialFieldIndex.UnitMeasureCode], ComparisonOperator.Equal, ((UnitMeasureEntity)unitMeasureInstance).UnitMeasureCode));
			}
			return selectFilter;
		}
