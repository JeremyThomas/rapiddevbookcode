...
public partial class Controls_ListCountryRegionCurrencyInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.CountryRegionCurrencyEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("CountryRegionCurrency.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		CountryRegionCurrencyEntity selectedEntity = (CountryRegionCurrencyEntity)_CountryRegionCurrencyDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&CountryRegionCode={0}", selectedEntity.CountryRegionCode);
		pkFieldsAndValues.AppendFormat("&CurrencyCode={0}", selectedEntity.CurrencyCode);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.CountryRegionCurrencyEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.CountryRegionCurrencyEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
	...
	public void SetContainingEntity(IEntityCore containingEntity, string name)
	{
		...
		{
			case "CountryRegionEntity":
				switch(name)
				{
					case "CountryRegionCurrencies":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(CountryRegionCurrencyFields.CountryRegionCode == ((CountryRegionEntity)containingEntity).CountryRegionCode);
						_CountryRegionCurrencyDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&CountryRegionCode=" + 
								((CountryRegionEntity)containingEntity).CountryRegionCode +
								"&20FkField=CountryRegionCode";
						break;
					default:
						break;
				}
			...
			case "CurrencyEntity":
				switch(name)
				{
					case "CountryRegionCurrencies":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(CountryRegionCurrencyFields.CurrencyCode == ((CurrencyEntity)containingEntity).CurrencyCode);
						_CountryRegionCurrencyDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&CurrencyCode=" + 
								((CurrencyEntity)containingEntity).CurrencyCode +
								"&20FkField=CurrencyCode";
						break;
					default:
						break;
				}
	...
	{
		set { _CountryRegionCurrencyDS.FilterToUse = value; }
	}
