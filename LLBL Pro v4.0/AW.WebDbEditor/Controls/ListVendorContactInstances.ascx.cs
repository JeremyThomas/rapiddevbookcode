...
public partial class Controls_ListVendorContactInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.VendorContactEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("VendorContact.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		VendorContactEntity selectedEntity = (VendorContactEntity)_VendorContactDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&ContactID={0}", selectedEntity.ContactID);
		pkFieldsAndValues.AppendFormat("&VendorID={0}", selectedEntity.VendorID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.VendorContactEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.VendorContactEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "ContactEntity":
				switch(name)
				{
					case "VendorContacts":
						...
						dsFilter.AddWithAnd(VendorContactFields.ContactID == ((ContactEntity)containingEntity).ContactID);
						_VendorContactDS.FilterToUse = dsFilter;
			...
			case "ContactTypeEntity":
				switch(name)
				{
					case "VendorContacts":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(VendorContactFields.ContactTypeID == ((ContactTypeEntity)containingEntity).ContactTypeID);
						_VendorContactDS.FilterToUse = dsFilter;
			...
			case "VendorEntity":
				switch(name)
				{
					case "VendorContacts":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(VendorContactFields.VendorID == ((VendorEntity)containingEntity).VendorID);
						_VendorContactDS.FilterToUse = dsFilter;
	...
	{
		set { _VendorContactDS.FilterToUse = value; }
	}
