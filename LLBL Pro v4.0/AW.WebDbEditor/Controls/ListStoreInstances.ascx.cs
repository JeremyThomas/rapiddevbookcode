...
public partial class Controls_ListStoreInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.StoreEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("Store.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		StoreEntity selectedEntity = (StoreEntity)_StoreDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&CustomerID={0}", selectedEntity.CustomerID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.StoreEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.StoreEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "ContactEntity":
				switch(name)
				{
					case "Stores":
						...
						_StoreDS.RelationsToUse = containingEntity.GetRelationsForFieldOfType("Stores");
						dsFilter.AddWithAnd(ContactFields.ContactID.SetObjectAlias("ContactEntity__") == ((ContactEntity)containingEntity).ContactID);
						_StoreDS.FilterToUse = dsFilter;
						break;
					default:
						break;
				}
			...
			case "SalesPersonEntity":
				switch(name)
				{
					case "Stores":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(StoreFields.SalesPersonID == ((SalesPersonEntity)containingEntity).EmployeeID);
						_StoreDS.FilterToUse = dsFilter;
	...
	{
		set { _StoreDS.FilterToUse = value; }
	}
