...
<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ListUnitMeasureInstances.ascx.cs" Inherits="Controls_ListUnitMeasureInstances" %>
...
<llblgenpro:LLBLGenProDataSource ID="_UnitMeasureDS" runat="server" DataContainerType="EntityCollection" EntityCollectionTypeName="AW.Data.CollectionClasses.UnitMeasureCollection, AW.Data" CacheLocation="Session" EnablePaging="True" />
<asp:GridView ID="theGrid" runat="server" SkinID="gridviewSkin" AutoGenerateColumns="False" DataKeyNames="UnitMeasureCode" 
					DataSourceID="_UnitMeasureDS" OnRowCommand="theGrid_RowCommand">
	<Columns>
		...
		<asp:TemplateField HeaderText="UnitMeasureCode" SortExpression="UnitMeasureCode"  HeaderStyle-HorizontalAlign="Left" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" >
			<ItemTemplate>
				<asp:Label ID="lblUnitMeasureCode" runat="server" Text='<%# Bind("UnitMeasureCode") %>'/>
			</ItemTemplate>
		</asp:TemplateField>
