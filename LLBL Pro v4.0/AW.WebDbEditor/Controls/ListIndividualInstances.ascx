...
<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ListIndividualInstances.ascx.cs" Inherits="Controls_ListIndividualInstances" %>
...
<llblgenpro:LLBLGenProDataSource ID="_IndividualDS" runat="server" DataContainerType="EntityCollection" EntityCollectionTypeName="AW.Data.CollectionClasses.IndividualCollection, AW.Data" CacheLocation="Session" EnablePaging="True" />
<asp:GridView ID="theGrid" runat="server" SkinID="gridviewSkin" AutoGenerateColumns="False" DataKeyNames="CustomerID" 
					DataSourceID="_IndividualDS" OnRowCommand="theGrid_RowCommand">
	<Columns>
		...
		<asp:TemplateField ShowHeader="False" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" HeaderStyle-CssClass="tableheaders" HeaderStyle-HorizontalAlign="Left">
			<ItemTemplate>
				<asp:Button ID="btnDeleteExisting" runat="server" CausesValidation="false" CommandArgument='<%# DataBinder.Eval(Container,"RowIndex") %>' CommandName="Delete" Text="Delete" SkinID="ButtonSkin" OnClientClick='<%# "return confirm(\"Are you sure you want to delete this Individual instance with PK values: "  + "CustomerID: " + Eval("CustomerID") + " ?\");" %>'/>
			</ItemTemplate>
		</asp:TemplateField>
		...
		<asp:TemplateField HeaderText="CustomerType" SortExpression="CustomerType"  HeaderStyle-HorizontalAlign="Left" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" >
			<ItemTemplate>
