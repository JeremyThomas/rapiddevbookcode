...
public partial class Controls_ListSalesPersonInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.SalesPersonEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("SalesPerson.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		SalesPersonEntity selectedEntity = (SalesPersonEntity)_SalesPersonDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&EmployeeID={0}", selectedEntity.EmployeeID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.SalesPersonEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.SalesPersonEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
	...
	public void SetContainingEntity(IEntityCore containingEntity, string name)
	{
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "SalesOrderHeaderEntity":
				...
				break;
			case "SalesPersonQuotaHistoryEntity":
				switch(name)
				{
					default:
						break;
			...
			case "SalesTerritoryEntity":
				switch(name)
				{
					case "SalesPeople":
						...
						dsFilter.AddWithAnd(SalesPersonFields.TerritoryID == ((SalesTerritoryEntity)containingEntity).TerritoryID);
						_SalesPersonDS.FilterToUse = dsFilter;
				...
				break;
			case "SalesTerritoryHistoryEntity":
				switch(name)
				{
					default:
						break;
	...
	{
		set { _SalesPersonDS.FilterToUse = value; }
	}
