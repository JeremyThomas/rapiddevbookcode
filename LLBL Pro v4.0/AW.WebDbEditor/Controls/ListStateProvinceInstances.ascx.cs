...
public partial class Controls_ListStateProvinceInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.StateProvinceEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("StateProvince.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		StateProvinceEntity selectedEntity = (StateProvinceEntity)_StateProvinceDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&StateProvinceID={0}", selectedEntity.StateProvinceID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.StateProvinceEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.StateProvinceEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			...
			case "CountryRegionEntity":
				switch(name)
				{
					case "StateProvinces":
						...
						dsFilter.AddWithAnd(StateProvinceFields.CountryRegionCode == ((CountryRegionEntity)containingEntity).CountryRegionCode);
						_StateProvinceDS.FilterToUse = dsFilter;
			...
			case "SalesTerritoryEntity":
				switch(name)
				{
					case "StateProvinces":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(StateProvinceFields.TerritoryID == ((SalesTerritoryEntity)containingEntity).TerritoryID);
						_StateProvinceDS.FilterToUse = dsFilter;
	...
	{
		set { _StateProvinceDS.FilterToUse = value; }
	}
