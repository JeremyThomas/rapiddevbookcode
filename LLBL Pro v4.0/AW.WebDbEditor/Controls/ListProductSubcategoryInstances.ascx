...
<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ListProductSubcategoryInstances.ascx.cs" Inherits="Controls_ListProductSubcategoryInstances" %>
...
<asp:GridView ID="theGrid" runat="server" SkinID="gridviewSkin" AutoGenerateColumns="False" DataKeyNames="ProductSubcategoryID" 
					DataSourceID="_ProductSubcategoryDS" OnRowCommand="theGrid_RowCommand">
	<Columns>
		...
		<asp:TemplateField HeaderText="ProductCategoryID" SortExpression="ProductCategoryID"  HeaderStyle-HorizontalAlign="Left" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" >
			<ItemTemplate>
				<asp:HyperLink ID="lnkProductCategoryID" runat="server" NavigateUrl='<%# "~/ViewExisting.aspx?EntityType=" + ((int)EntityType.ProductCategoryEntity).ToString() + "&ProductCategoryID=" + Eval("ProductCategoryID")%>' Text='<%# Bind("ProductCategoryID") %>'/>
			</ItemTemplate>
		</asp:TemplateField>
		<asp:TemplateField HeaderText="ProductSubcategoryID" SortExpression="ProductSubcategoryID" ItemStyle-HorizontalAlign="Right" HeaderStyle-HorizontalAlign="Right" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" >
			<ItemTemplate>
				<asp:Label ID="lblProductSubcategoryID" runat="server" Text='<%# Bind("ProductSubcategoryID") %>'/>
			</ItemTemplate>
		</asp:TemplateField>
