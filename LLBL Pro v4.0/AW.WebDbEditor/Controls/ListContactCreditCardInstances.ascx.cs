...
public partial class Controls_ListContactCreditCardInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.ContactCreditCardEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("ContactCreditCard.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		ContactCreditCardEntity selectedEntity = (ContactCreditCardEntity)_ContactCreditCardDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&ContactID={0}", selectedEntity.ContactID);
		pkFieldsAndValues.AppendFormat("&CreditCardID={0}", selectedEntity.CreditCardID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.ContactCreditCardEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.ContactCreditCardEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "ContactEntity":
				switch(name)
				{
					case "ContactCreditCards":
						...
						dsFilter.AddWithAnd(ContactCreditCardFields.ContactID == ((ContactEntity)containingEntity).ContactID);
						_ContactCreditCardDS.FilterToUse = dsFilter;
			...
			case "CreditCardEntity":
				switch(name)
				{
					case "ContactCreditCards":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(ContactCreditCardFields.CreditCardID == ((CreditCardEntity)containingEntity).CreditCardID);
						_ContactCreditCardDS.FilterToUse = dsFilter;
	...
	{
		set { _ContactCreditCardDS.FilterToUse = value; }
	}
