...
public partial class Controls_ListContactInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.ContactEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("Contact.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		ContactEntity selectedEntity = (ContactEntity)_ContactDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&ContactID={0}", selectedEntity.ContactID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.ContactEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.ContactEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
			...
			case "StoreEntity":
				switch(name)
				{
					case "Contacts":
						...
						_ContactDS.RelationsToUse = containingEntity.GetRelationsForFieldOfType("Contacts");
						dsFilter.AddWithAnd(StoreFields.CustomerID.SetObjectAlias("StoreEntity__") == ((StoreEntity)containingEntity).CustomerID);
						_ContactDS.FilterToUse = dsFilter;
						break;
					default:
						break;
				}
				break;
			case "VendorContactEntity":
				switch(name)
				{
					default:
						break;
	...
	{
		set { _ContactDS.FilterToUse = value; }
	}
