...
public partial class Controls_ListTransactionHistoryInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.TransactionHistoryEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("TransactionHistory.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		TransactionHistoryEntity selectedEntity = (TransactionHistoryEntity)_TransactionHistoryDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&TransactionID={0}", selectedEntity.TransactionID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.TransactionHistoryEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.TransactionHistoryEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
...
IPredicateExpression dsFilter;
			...
			case "ProductEntity":
				switch(name)
				{
					case "TransactionHistories":
						...
						dsFilter.AddWithAnd(TransactionHistoryFields.ProductID == ((ProductEntity)containingEntity).ProductID);
						_TransactionHistoryDS.FilterToUse = dsFilter;
	...
	{
		set { _TransactionHistoryDS.FilterToUse = value; }
	}
