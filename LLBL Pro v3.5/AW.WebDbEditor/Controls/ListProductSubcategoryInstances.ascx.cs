...
public partial class Controls_ListProductSubcategoryInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.ProductSubcategoryEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("ProductSubcategory.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		ProductSubcategoryEntity selectedEntity = (ProductSubcategoryEntity)_ProductSubcategoryDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&ProductSubcategoryID={0}", selectedEntity.ProductSubcategoryID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.ProductSubcategoryEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.ProductSubcategoryEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
			...
			case "ProductCategoryEntity":
				switch(name)
				{
					case "ProductSubcategories":
						...
						dsFilter.AddWithAnd(ProductSubcategoryFields.ProductCategoryID == ((ProductCategoryEntity)containingEntity).ProductCategoryID);
						_ProductSubcategoryDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&ProductCategoryID=" + 
								((ProductCategoryEntity)containingEntity).ProductCategoryID +
								"&20FkField=ProductCategoryID";
						break;
					default:
						break;
				}
	...
	{
		set { _ProductSubcategoryDS.FilterToUse = value; }
	}
