...
<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ListIllustrationInstances.ascx.cs" Inherits="Controls_ListIllustrationInstances" %>
...
<llblgenpro:LLBLGenProDataSource ID="_IllustrationDS" runat="server" DataContainerType="EntityCollection" EntityCollectionTypeName="AW.Data.CollectionClasses.IllustrationCollection, AW.Data" CacheLocation="Session" EnablePaging="True" />
<asp:GridView ID="theGrid" runat="server" SkinID="gridviewSkin" AutoGenerateColumns="False" DataKeyNames="IllustrationID" 
					DataSourceID="_IllustrationDS" OnRowCommand="theGrid_RowCommand">
	<Columns>
		...
		<asp:TemplateField ShowHeader="False" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" HeaderStyle-CssClass="tableheaders" HeaderStyle-HorizontalAlign="Left">
			<ItemTemplate>
				<asp:Button ID="btnDeleteExisting" runat="server" CausesValidation="false" CommandArgument='<%# DataBinder.Eval(Container,"RowIndex") %>' CommandName="Delete" Text="Delete" SkinID="ButtonSkin" OnClientClick='<%# "return confirm(\"Are you sure you want to delete this Illustration instance with PK values: "  + "IllustrationID: " + Eval("IllustrationID") + " ?\");" %>'/>
			</ItemTemplate>
		</asp:TemplateField>
		<asp:TemplateField HeaderText="IllustrationID" SortExpression="IllustrationID" ItemStyle-HorizontalAlign="Right" HeaderStyle-HorizontalAlign="Right" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" >
			<ItemTemplate>
				<asp:Label ID="lblIllustrationID" runat="server" Text='<%# Bind("IllustrationID") %>'/>
			</ItemTemplate>
		</asp:TemplateField>
