...
namespace AW.Data.EntityClasses
{
		...
		// __LLBLGENPRO_USER_CODE_REGION_END	
	{
		...
		{
			this.PerformDesetupSyncRelatedEntity( _product, new PropertyChangedEventHandler( OnProductPropertyChanged ), "Product", TransactionHistoryEntity.Relations.ProductEntityUsingProductID, true, signalRelatedEntity, "TransactionHistories", resetFKFields, new int[] { (int)TransactionHistoryFieldIndex.ProductID } );		
			_product = null;
		}
		...
		private void SetupSyncProduct(IEntity relatedEntity)
		{
			...
			{		
				...
				this.PerformSetupSyncRelatedEntity( _product, new PropertyChangedEventHandler( OnProductPropertyChanged ), "Product", TransactionHistoryEntity.Relations.ProductEntityUsingProductID, true, ref _alreadyFetchedProduct, new string[] {  } );
			}
		}
		...
		private bool Fetch(System.Int32 transactionID, IPrefetchPath prefetchPathToUse, Context contextToUse, ExcludeIncludeFieldsList excludedIncludedFields)
		{
			...
			{
				OnFetch();
				this.Fields[(int)TransactionHistoryFieldIndex.TransactionID].ForcedCurrentValueWrite(transactionID);
