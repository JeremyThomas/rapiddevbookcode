...
public partial class Controls_ListEmployeePayHistoryInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.EmployeePayHistoryEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("EmployeePayHistory.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		EmployeePayHistoryEntity selectedEntity = (EmployeePayHistoryEntity)_EmployeePayHistoryDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&EmployeeID={0}", selectedEntity.EmployeeID);
		pkFieldsAndValues.AppendFormat("&RateChangeDate={0}", selectedEntity.RateChangeDate);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.EmployeePayHistoryEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.EmployeePayHistoryEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
...
IPredicateExpression dsFilter;
			...
			case "EmployeeEntity":
				switch(name)
				{
					case "EmployeePayHistories":
						...
						dsFilter.AddWithAnd(EmployeePayHistoryFields.EmployeeID == ((EmployeeEntity)containingEntity).EmployeeID);
						_EmployeePayHistoryDS.FilterToUse = dsFilter;
	...
	{
		set { _EmployeePayHistoryDS.FilterToUse = value; }
	}
