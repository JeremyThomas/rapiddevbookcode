...
public partial class Controls_ListPurchaseOrderHeaderInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.PurchaseOrderHeaderEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("PurchaseOrderHeader.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		PurchaseOrderHeaderEntity selectedEntity = (PurchaseOrderHeaderEntity)_PurchaseOrderHeaderDS.EntityCollection[index];
		...
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.PurchaseOrderHeaderEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.PurchaseOrderHeaderEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "EmployeeEntity":
				switch(name)
				{
					case "PurchaseOrderHeaders":
						...
						dsFilter.AddWithAnd(PurchaseOrderHeaderFields.EmployeeID == ((EmployeeEntity)containingEntity).EmployeeID);
						_PurchaseOrderHeaderDS.FilterToUse = dsFilter;
				...
				break;
			case "PurchaseOrderDetailEntity":
				switch(name)
				{
					default:
						break;
			...
			case "ShipMethodEntity":
				switch(name)
				{
					case "PurchaseOrderHeaders":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(PurchaseOrderHeaderFields.ShipMethodID == ((ShipMethodEntity)containingEntity).ShipMethodID);
						_PurchaseOrderHeaderDS.FilterToUse = dsFilter;
			...
			case "VendorEntity":
				switch(name)
				{
					case "PurchaseOrderHeaders":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(PurchaseOrderHeaderFields.VendorID == ((VendorEntity)containingEntity).VendorID);
						_PurchaseOrderHeaderDS.FilterToUse = dsFilter;
	...
	{
		set { _PurchaseOrderHeaderDS.FilterToUse = value; }
	}
