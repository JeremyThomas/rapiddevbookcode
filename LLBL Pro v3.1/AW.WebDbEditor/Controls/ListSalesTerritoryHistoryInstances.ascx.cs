...
public partial class Controls_ListSalesTerritoryHistoryInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.SalesTerritoryHistoryEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("SalesTerritoryHistory.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		SalesTerritoryHistoryEntity selectedEntity = (SalesTerritoryHistoryEntity)_SalesTerritoryHistoryDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&SalesPersonID={0}", selectedEntity.SalesPersonID);
		...
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.SalesTerritoryHistoryEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.SalesTerritoryHistoryEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "SalesPersonEntity":
				switch(name)
				{
					case "SalesTerritoryHistories":
						...
						dsFilter.AddWithAnd(SalesTerritoryHistoryFields.SalesPersonID == ((SalesPersonEntity)containingEntity).EmployeeID);
						_SalesTerritoryHistoryDS.FilterToUse = dsFilter;
			...
			case "SalesTerritoryEntity":
				switch(name)
				{
					case "SalesTerritoryHistories":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(SalesTerritoryHistoryFields.TerritoryID == ((SalesTerritoryEntity)containingEntity).TerritoryID);
						_SalesTerritoryHistoryDS.FilterToUse = dsFilter;
	...
	{
		set { _SalesTerritoryHistoryDS.FilterToUse = value; }
	}
