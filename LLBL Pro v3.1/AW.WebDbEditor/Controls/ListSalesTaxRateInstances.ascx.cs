...
public partial class Controls_ListSalesTaxRateInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.SalesTaxRateEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("SalesTaxRate.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		SalesTaxRateEntity selectedEntity = (SalesTaxRateEntity)_SalesTaxRateDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&SalesTaxRateID={0}", selectedEntity.SalesTaxRateID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.SalesTaxRateEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.SalesTaxRateEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
...
IPredicateExpression dsFilter;
		...
		{
			case "StateProvinceEntity":
				switch(name)
				{
					case "SalesTaxRates":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(SalesTaxRateFields.StateProvinceID == ((StateProvinceEntity)containingEntity).StateProvinceID);
						_SalesTaxRateDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&StateProvinceID=" + 
								((StateProvinceEntity)containingEntity).StateProvinceID +
								"&20FkField=StateProvinceID";
						break;
					default:
						break;
				}
	...
	{
		set { _SalesTaxRateDS.FilterToUse = value; }
	}
