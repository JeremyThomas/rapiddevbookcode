...
public partial class Controls_ListSpecialOfferProductInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.SpecialOfferProductEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("SpecialOfferProduct.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		SpecialOfferProductEntity selectedEntity = (SpecialOfferProductEntity)_SpecialOfferProductDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&ProductID={0}", selectedEntity.ProductID);
		pkFieldsAndValues.AppendFormat("&SpecialOfferID={0}", selectedEntity.SpecialOfferID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.SpecialOfferProductEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.SpecialOfferProductEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "ProductEntity":
				switch(name)
				{
					case "SpecialOfferProducts":
						...
						dsFilter.AddWithAnd(SpecialOfferProductFields.ProductID == ((ProductEntity)containingEntity).ProductID);
						_SpecialOfferProductDS.FilterToUse = dsFilter;
			...
			case "SpecialOfferEntity":
				switch(name)
				{
					case "SpecialOfferProducts":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(SpecialOfferProductFields.SpecialOfferID == ((SpecialOfferEntity)containingEntity).SpecialOfferID);
						_SpecialOfferProductDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&SpecialOfferID=" + 
								((SpecialOfferEntity)containingEntity).SpecialOfferID +
								"&20FkField=SpecialOfferID";
						break;
					default:
						break;
				}
	...
	{
		set { _SpecialOfferProductDS.FilterToUse = value; }
	}
