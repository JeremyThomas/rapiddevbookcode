...
public partial class Controls_ListCustomerAddressInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.CustomerAddressEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("CustomerAddress.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		CustomerAddressEntity selectedEntity = (CustomerAddressEntity)_CustomerAddressDS.EntityCollection[index];
		...
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.CustomerAddressEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.CustomerAddressEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "AddressEntity":
				switch(name)
				{
					case "CustomerAddresses":
						...
						dsFilter.AddWithAnd(CustomerAddressFields.AddressID == ((AddressEntity)containingEntity).AddressID);
						_CustomerAddressDS.FilterToUse = dsFilter;
			...
			case "AddressTypeEntity":
				switch(name)
				{
					case "CustomerAddresses":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(CustomerAddressFields.AddressTypeID == ((AddressTypeEntity)containingEntity).AddressTypeID);
						_CustomerAddressDS.FilterToUse = dsFilter;
			...
			case "CustomerEntity":
				switch(name)
				{
					case "CustomerAddresses":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(CustomerAddressFields.CustomerID == ((CustomerEntity)containingEntity).CustomerID);
						_CustomerAddressDS.FilterToUse = dsFilter;
	...
	{
		set { _CustomerAddressDS.FilterToUse = value; }
	}
