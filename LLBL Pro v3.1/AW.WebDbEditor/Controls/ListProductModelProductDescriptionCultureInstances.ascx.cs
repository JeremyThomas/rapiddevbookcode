...
public partial class Controls_ListProductModelProductDescriptionCultureInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.ProductModelProductDescriptionCultureEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("ProductModelProductDescriptionCulture.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		ProductModelProductDescriptionCultureEntity selectedEntity = (ProductModelProductDescriptionCultureEntity)_ProductModelProductDescriptionCultureDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&CultureID={0}", selectedEntity.CultureID);
		pkFieldsAndValues.AppendFormat("&ProductDescriptionID={0}", selectedEntity.ProductDescriptionID);
		pkFieldsAndValues.AppendFormat("&ProductModelID={0}", selectedEntity.ProductModelID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.ProductModelProductDescriptionCultureEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.ProductModelProductDescriptionCultureEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "CultureEntity":
				switch(name)
				{
					case "ProductModelProductDescriptionCultures":
						...
						dsFilter.AddWithAnd(ProductModelProductDescriptionCultureFields.CultureID == ((CultureEntity)containingEntity).CultureID);
						_ProductModelProductDescriptionCultureDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&CultureID=" + 
								((CultureEntity)containingEntity).CultureID +
								"&20FkField=CultureID";
						break;
					default:
						break;
				...
				break;
			case "ProductDescriptionEntity":
				switch(name)
				{
					case "ProductModelProductDescriptionCultures":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(ProductModelProductDescriptionCultureFields.ProductDescriptionID == ((ProductDescriptionEntity)containingEntity).ProductDescriptionID);
						_ProductModelProductDescriptionCultureDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&ProductDescriptionID=" + 
								((ProductDescriptionEntity)containingEntity).ProductDescriptionID +
								"&20FkField=ProductDescriptionID";
						break;
					default:
						break;
				}
			...
			case "ProductModelEntity":
				switch(name)
				{
					case "ProductModelProductDescriptionCultures":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(ProductModelProductDescriptionCultureFields.ProductModelID == ((ProductModelEntity)containingEntity).ProductModelID);
						_ProductModelProductDescriptionCultureDS.FilterToUse = dsFilter;
	...
	{
		set { _ProductModelProductDescriptionCultureDS.FilterToUse = value; }
	}
