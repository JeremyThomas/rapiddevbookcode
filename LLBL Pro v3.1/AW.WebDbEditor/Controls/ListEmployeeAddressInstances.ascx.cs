...
public partial class Controls_ListEmployeeAddressInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.EmployeeAddressEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("EmployeeAddress.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		EmployeeAddressEntity selectedEntity = (EmployeeAddressEntity)_EmployeeAddressDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&AddressID={0}", selectedEntity.AddressID);
		pkFieldsAndValues.AppendFormat("&EmployeeID={0}", selectedEntity.EmployeeID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.EmployeeAddressEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.EmployeeAddressEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "AddressEntity":
				switch(name)
				{
					case "EmployeeAddresses":
						...
						dsFilter.AddWithAnd(EmployeeAddressFields.AddressID == ((AddressEntity)containingEntity).AddressID);
						_EmployeeAddressDS.FilterToUse = dsFilter;
			...
			case "EmployeeEntity":
				switch(name)
				{
					case "EmployeeAddresses":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(EmployeeAddressFields.EmployeeID == ((EmployeeEntity)containingEntity).EmployeeID);
						_EmployeeAddressDS.FilterToUse = dsFilter;
	...
	{
		set { _EmployeeAddressDS.FilterToUse = value; }
	}
