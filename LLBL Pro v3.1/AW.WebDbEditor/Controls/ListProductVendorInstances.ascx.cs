...
public partial class Controls_ListProductVendorInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.ProductVendorEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("ProductVendor.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		ProductVendorEntity selectedEntity = (ProductVendorEntity)_ProductVendorDS.EntityCollection[index];
		...
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.ProductVendorEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.ProductVendorEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "ProductEntity":
				switch(name)
				{
					case "ProductVendors":
						...
						dsFilter.AddWithAnd(ProductVendorFields.ProductID == ((ProductEntity)containingEntity).ProductID);
						_ProductVendorDS.FilterToUse = dsFilter;
			...
			case "UnitMeasureEntity":
				switch(name)
				{
					case "ProductVendors":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(ProductVendorFields.UnitMeasureCode == ((UnitMeasureEntity)containingEntity).UnitMeasureCode);
						_ProductVendorDS.FilterToUse = dsFilter;
			...
			case "VendorEntity":
				switch(name)
				{
					case "ProductVendors":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(ProductVendorFields.VendorID == ((VendorEntity)containingEntity).VendorID);
						_ProductVendorDS.FilterToUse = dsFilter;
	...
	{
		set { _ProductVendorDS.FilterToUse = value; }
	}
