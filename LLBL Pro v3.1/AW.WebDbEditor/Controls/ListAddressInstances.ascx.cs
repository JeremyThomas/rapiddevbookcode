...
public partial class Controls_ListAddressInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.AddressEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("Address.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		AddressEntity selectedEntity = (AddressEntity)_AddressDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&AddressID={0}", selectedEntity.AddressID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.AddressEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.AddressEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
			...
			case "StateProvinceEntity":
				switch(name)
				{
					case "Addresses":
						...
						dsFilter.AddWithAnd(AddressFields.StateProvinceID == ((StateProvinceEntity)containingEntity).StateProvinceID);
						_AddressDS.FilterToUse = dsFilter;
	...
	{
		set { _AddressDS.FilterToUse = value; }
	}
