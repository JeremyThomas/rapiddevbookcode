...
namespace AW.Win
{
  public partial class frmOrganization : Form
  {
    ...
    private static TreeNode GetManagersRecursive(EmployeeEntity Employee)
    {
    ...
    private static TreeNode GetEmployeesRecursive(EmployeeEntity Employee)
    {
      TreeNode EmployeeNode = MakeNode(Employee);
      if (Employee.Manages.Count > 0)
      {
    ...
    private void btnSearchNon_Click(object sender, EventArgs e)
    {
      ...
      EmployeeEntity emp = new EmployeeEntity(Convert.ToInt32(cbEmployee.SelectedValue));
