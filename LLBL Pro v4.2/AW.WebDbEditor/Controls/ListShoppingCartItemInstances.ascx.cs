...
public partial class Controls_ListShoppingCartItemInstances : System.Web.UI.UserControl, IListControl
{
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("ShoppingCartItem.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		ShoppingCartItemEntity selectedEntity = (ShoppingCartItemEntity)_ShoppingCartItemDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&ShoppingCartItemID={0}", selectedEntity.ShoppingCartItemID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.ShoppingCartItemEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.ShoppingCartItemEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
