...
public partial class Controls_ListWorkOrderRoutingInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.WorkOrderRoutingEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("WorkOrderRouting.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		WorkOrderRoutingEntity selectedEntity = (WorkOrderRoutingEntity)_WorkOrderRoutingDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&OperationSequence={0}", selectedEntity.OperationSequence);
		pkFieldsAndValues.AppendFormat("&ProductID={0}", selectedEntity.ProductID);
		pkFieldsAndValues.AppendFormat("&WorkOrderID={0}", selectedEntity.WorkOrderID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.WorkOrderRoutingEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.WorkOrderRoutingEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "LocationEntity":
				switch(name)
				{
					case "WorkOrderRoutings":
						...
						dsFilter.AddWithAnd(WorkOrderRoutingFields.LocationID == ((LocationEntity)containingEntity).LocationID);
						_WorkOrderRoutingDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&LocationID=" + 
								((LocationEntity)containingEntity).LocationID +
								"&20FkField=LocationID";
						break;
					default:
						break;
				}
			...
			case "WorkOrderEntity":
				switch(name)
				{
					case "WorkOrderRoutings":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(WorkOrderRoutingFields.WorkOrderID == ((WorkOrderEntity)containingEntity).WorkOrderID);
						_WorkOrderRoutingDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&WorkOrderID=" + 
								((WorkOrderEntity)containingEntity).WorkOrderID +
								"&20FkField=WorkOrderID";
						break;
					default:
						break;
				}
	...
	{
		set { _WorkOrderRoutingDS.FilterToUse = value; }
	}
