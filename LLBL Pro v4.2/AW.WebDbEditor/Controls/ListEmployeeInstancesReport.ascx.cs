...
public partial class Controls_ListEmployeeInstancesReport : System.Web.UI.UserControl, IListControl
{
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "ContactEntity":
				switch(name)
				{
					case "Employees":
						...
						dsFilter.AddWithAnd(EmployeeFields.ContactID == ((ContactEntity)containingEntity).ContactID);
						_EmployeeDS.FilterToUse = dsFilter;
			...
			case "EmployeeEntity":
				switch(name)
				{
					case "Manages":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(EmployeeFields.ManagerID == ((EmployeeEntity)containingEntity).EmployeeID);
						_EmployeeDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&ManagerID=" + 
