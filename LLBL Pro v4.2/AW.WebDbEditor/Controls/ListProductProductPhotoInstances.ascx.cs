...
public partial class Controls_ListProductProductPhotoInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.ProductProductPhotoEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("ProductProductPhoto.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		ProductProductPhotoEntity selectedEntity = (ProductProductPhotoEntity)_ProductProductPhotoDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&ProductID={0}", selectedEntity.ProductID);
		pkFieldsAndValues.AppendFormat("&ProductPhotoID={0}", selectedEntity.ProductPhotoID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.ProductProductPhotoEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.ProductProductPhotoEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
			...
			case "ProductEntity":
				switch(name)
				{
					case "ProductProductPhotos":
						...
						dsFilter.AddWithAnd(ProductProductPhotoFields.ProductID == ((ProductEntity)containingEntity).ProductID);
						_ProductProductPhotoDS.FilterToUse = dsFilter;
				...
				break;
			case "ProductPhotoEntity":
				switch(name)
				{
					case "ProductProductPhotos":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(ProductProductPhotoFields.ProductPhotoID == ((ProductPhotoEntity)containingEntity).ProductPhotoID);
						_ProductProductPhotoDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&ProductPhotoID=" + 
								((ProductPhotoEntity)containingEntity).ProductPhotoID +
								"&20FkField=ProductPhotoID";
						break;
					default:
						break;
				}
	...
	{
		set { _ProductProductPhotoDS.FilterToUse = value; }
	}
