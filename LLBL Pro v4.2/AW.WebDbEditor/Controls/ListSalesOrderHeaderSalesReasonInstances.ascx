...
<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ListSalesOrderHeaderSalesReasonInstances.ascx.cs" Inherits="Controls_ListSalesOrderHeaderSalesReasonInstances" %>
...
<llblgenpro:LLBLGenProDataSource ID="_SalesOrderHeaderSalesReasonDS" runat="server" DataContainerType="EntityCollection" EntityCollectionTypeName="AW.Data.CollectionClasses.SalesOrderHeaderSalesReasonCollection, AW.Data" CacheLocation="Session" EnablePaging="True" />
<asp:GridView ID="theGrid" runat="server" SkinID="gridviewSkin" AutoGenerateColumns="False" DataKeyNames="SalesOrderID, SalesReasonID" 
					DataSourceID="_SalesOrderHeaderSalesReasonDS" OnRowCommand="theGrid_RowCommand">
	<Columns>
		...
		<asp:TemplateField HeaderText="SalesReasonID" SortExpression="SalesReasonID"  HeaderStyle-HorizontalAlign="Left" ItemStyle-CssClass="ColumnContent" ItemStyle-Wrap="false" >
			<ItemTemplate>
				<asp:HyperLink ID="lnkSalesReasonID" runat="server" NavigateUrl='<%# "~/ViewExisting.aspx?EntityType=" + ((int)EntityType.SalesReasonEntity).ToString() + "&SalesReasonID=" + Eval("SalesReasonID")%>' Text='<%# Bind("SalesReasonID") %>'/>
			</ItemTemplate>
		</asp:TemplateField>
