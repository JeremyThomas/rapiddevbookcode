...
namespace AW.Winforms.Helpers.ConnectionUI
{
  ...
  public class DataConnectionConfiguration : IDataConnectionConfiguration
  {
    ...
    {
      var dataDataSourceProvider = dcd.DataSources.SelectMany(ds => ds.Providers, (ds, p) => new {ds, p}).FirstOrDefault(@t => @t.p.Name == provider);
      if (dataDataSourceProvider != null)
      {
        dcd.SelectedDataSource = dataDataSourceProvider.ds;
        dcd.SelectedDataProvider = dataDataSourceProvider.p;
        return true;
      }
      return false;
    }
    ...
    public void LoadConfiguration(DataConnectionDialog dialog)
    {
      ...
      CreateAndAddPseudoOracleDataProvider(dialog, true);
      CreateAndAddPseudoOracleDataProvider(dialog, false);
    ...
    private static void CreateAndAddPseudoOracleDataProvider(DataConnectionDialog dialog, bool managed)
    {
      var pseudoOracleDataProvider = CreatePseudoOracleDataProvider(managed);
      DataSource.OracleDataSource.Providers.Add(pseudoOracleDataProvider);
      dialog.UnspecifiedDataSource.Providers.Add(pseudoOracleDataProvider);
    }

    private static DataProvider CreatePseudoOracleDataProvider(bool managed)
    {
      var oracleProvider = DataSource.OracleDataSource.DefaultProvider;

      var description = managed ? "Use this data provider to connect to Oracle Database using the ODP.NET, Managed Driver" : "Use this data provider to connect to Oracle Database using the Oracle Data Provider for .NET";
      var displayName = managed ? "ODP.NET, Managed Driver" : "ODP.NET, Unmanaged Driver";
      var oracleDataProvider = new DataProvider(managed ? "Oracle.ManagedDataAccess.Client" : "Oracle.DataAccess.Client",
        displayName, displayName, description,
        oracleProvider.TargetConnectionType, new Dictionary<string, string>
        {
          {
