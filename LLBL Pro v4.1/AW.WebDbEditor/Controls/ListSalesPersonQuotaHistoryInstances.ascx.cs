...
public partial class Controls_ListSalesPersonQuotaHistoryInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.SalesPersonQuotaHistoryEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("SalesPersonQuotaHistory.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		SalesPersonQuotaHistoryEntity selectedEntity = (SalesPersonQuotaHistoryEntity)_SalesPersonQuotaHistoryDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&QuotaDate={0}", selectedEntity.QuotaDate);
		pkFieldsAndValues.AppendFormat("&SalesPersonID={0}", selectedEntity.SalesPersonID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.SalesPersonQuotaHistoryEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.SalesPersonQuotaHistoryEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
...
IPredicateExpression dsFilter;
			...
			case "SalesPersonEntity":
				switch(name)
				{
					case "SalesPersonQuotaHistories":
						...
						dsFilter.AddWithAnd(SalesPersonQuotaHistoryFields.SalesPersonID == ((SalesPersonEntity)containingEntity).EmployeeID);
						_SalesPersonQuotaHistoryDS.FilterToUse = dsFilter;
	...
	{
		set { _SalesPersonQuotaHistoryDS.FilterToUse = value; }
	}
