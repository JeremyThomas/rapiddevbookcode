...
public partial class Controls_ListIndividualInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.IndividualEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("Individual.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		IndividualEntity selectedEntity = (IndividualEntity)_IndividualDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&CustomerID={0}", selectedEntity.CustomerID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.IndividualEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.IndividualEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
...
IPredicateExpression dsFilter;
			...
			case "ContactEntity":
				switch(name)
				{
					case "Individuals":
						...
						dsFilter.AddWithAnd(IndividualFields.ContactID == ((ContactEntity)containingEntity).ContactID);
						_IndividualDS.FilterToUse = dsFilter;
	...
	{
		set { _IndividualDS.FilterToUse = value; }
	}
