...
public partial class Controls_ListPurchaseOrderDetailInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.PurchaseOrderDetailEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("PurchaseOrderDetail.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		PurchaseOrderDetailEntity selectedEntity = (PurchaseOrderDetailEntity)_PurchaseOrderDetailDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&PurchaseOrderDetailID={0}", selectedEntity.PurchaseOrderDetailID);
		pkFieldsAndValues.AppendFormat("&PurchaseOrderID={0}", selectedEntity.PurchaseOrderID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.PurchaseOrderDetailEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.PurchaseOrderDetailEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
			...
			case "ProductEntity":
				switch(name)
				{
					case "PurchaseOrderDetails":
						...
						dsFilter.AddWithAnd(PurchaseOrderDetailFields.ProductID == ((ProductEntity)containingEntity).ProductID);
						_PurchaseOrderDetailDS.FilterToUse = dsFilter;
				...
				break;
			case "PurchaseOrderHeaderEntity":
				switch(name)
				{
					case "PurchaseOrderDetails":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(PurchaseOrderDetailFields.PurchaseOrderID == ((PurchaseOrderHeaderEntity)containingEntity).PurchaseOrderID);
						_PurchaseOrderDetailDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&PurchaseOrderID=" + 
								((PurchaseOrderHeaderEntity)containingEntity).PurchaseOrderID +
								"&20FkField=PurchaseOrderID";
						break;
					default:
						break;
				}
	...
	{
		set { _PurchaseOrderDetailDS.FilterToUse = value; }
	}
