...
public partial class Controls_ListProductDocumentInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.ProductDocumentEntity + additionalFiltersAsString);
	}
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
		...
		// export to excel
        GridViewExportUtils.Export("ProductDocument.xls", theGrid);
    }
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		ProductDocumentEntity selectedEntity = (ProductDocumentEntity)_ProductDocumentDS.EntityCollection[index];
		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&DocumentID={0}", selectedEntity.DocumentID);
		pkFieldsAndValues.AppendFormat("&ProductID={0}", selectedEntity.ProductID);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.ProductDocumentEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.ProductDocumentEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
		...
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "DocumentEntity":
				switch(name)
				{
					case "ProductDocuments":
						...
						dsFilter.AddWithAnd(ProductDocumentFields.DocumentID == ((DocumentEntity)containingEntity).DocumentID);
						_ProductDocumentDS.FilterToUse = dsFilter;
						ViewState["additionalFilters"] = "&DocumentID=" + 
								((DocumentEntity)containingEntity).DocumentID +
								"&20FkField=DocumentID";
						break;
					default:
						break;
				}
			...
			case "ProductEntity":
				switch(name)
				{
					case "ProductDocuments":
						dsFilter = new PredicateExpression();
						dsFilter.AddWithAnd(ProductDocumentFields.ProductID == ((ProductEntity)containingEntity).ProductID);
						_ProductDocumentDS.FilterToUse = dsFilter;
	...
	{
		set { _ProductDocumentDS.FilterToUse = value; }
	}
