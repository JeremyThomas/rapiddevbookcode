...
{
    [ServiceContract()]
  public interface IRemoteDataAccessAdapter : IDataAccessAdapter

    {
      [OperationContract]
      new void Commit();
       [OperationContract]
       new void FetchEntityCollection(IEntityCollection2 collectionToFill, IRelationPredicateBucket filterBucket);
        [OperationContract] 
      new void FetchProjection(List<IDataValueProjector> valueProjectors, IGeneralDataProjector projector, QueryParameters parameters);
       new FunctionMappingStore FunctionMappings { [OperationContract] get; }
    }
