...
namespace Northwind.DAL.EntityClasses
{
		...
		// __LLBLGENPRO_USER_CODE_REGION_END	
	{
		...
		protected override void AddInternalsToContext()
		{
			...
			{
				_employeeTerritories.ActiveContext = this.ActiveContext;
			}
			if(_region!=null)
			{
				_region.ActiveContext = this.ActiveContext;
			}
		}
		...
		{
			this.PerformDesetupSyncRelatedEntity( _region, new PropertyChangedEventHandler( OnRegionPropertyChanged ), "Region", Relations.RegionEntityUsingRegionId, true, signalRelatedEntity, "Territories", resetFKFields, new int[] { (int)TerritoryFieldIndex.RegionId } );
			_region = null;
		}

		/// <summary> setups the sync logic for member _region</summary>
		/// <param name="relatedEntity">Instance to set as the related entity of type entityType</param>
		private void SetupSyncRegion(IEntity2 relatedEntity)
		{
			if(_region!=relatedEntity)
			{
				DesetupSyncRegion(true, true);
				_region = (RegionEntity)relatedEntity;
				this.PerformSetupSyncRelatedEntity( _region, new PropertyChangedEventHandler( OnRegionPropertyChanged ), "Region", Relations.RegionEntityUsingRegionId, true, new string[] {  } );
			}
		}
