...
namespace Northwind.DAL.EntityClasses
{
		...
		// __LLBLGENPRO_USER_CODE_REGION_END	
	{
		...
		{
			this.PerformDesetupSyncRelatedEntity( _employee, new PropertyChangedEventHandler( OnEmployeePropertyChanged ), "Employee", Relations.EmployeeEntityUsingEmployeeIdReportsTo, true, signalRelatedEntity, "Employees", resetFKFields, new int[] { (int)EmployeeFieldIndex.ReportsTo } );
			_employee = null;
		}
		...
		private void SetupSyncEmployee(IEntity2 relatedEntity)
		{
			if(_employee!=relatedEntity)
			{
				...
				this.PerformSetupSyncRelatedEntity( _employee, new PropertyChangedEventHandler( OnEmployeePropertyChanged ), "Employee", Relations.EmployeeEntityUsingEmployeeIdReportsTo, true, new string[] {  } );
			}
		}
