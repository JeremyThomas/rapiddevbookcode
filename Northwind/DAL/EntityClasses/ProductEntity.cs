...
namespace Northwind.DAL.EntityClasses
{
		...
		// __LLBLGENPRO_USER_CODE_REGION_END	
	{
		...
		protected override void AddInternalsToContext()
		{
			...
			if(_category!=null)
			{
				_category.ActiveContext = this.ActiveContext;
			}
			if(_supplier!=null)
			{
				_supplier.ActiveContext = this.ActiveContext;
			}
		}
		...
		{
			this.PerformDesetupSyncRelatedEntity( _category, new PropertyChangedEventHandler( OnCategoryPropertyChanged ), "Category", Relations.CategoryEntityUsingCategoryId, true, signalRelatedEntity, "Products", resetFKFields, new int[] { (int)ProductFieldIndex.CategoryId } );
			_category = null;
		}

		/// <summary> setups the sync logic for member _category</summary>
		/// <param name="relatedEntity">Instance to set as the related entity of type entityType</param>
		private void SetupSyncCategory(IEntity2 relatedEntity)
		{
			if(_category!=relatedEntity)
			{
				DesetupSyncCategory(true, true);
				_category = (CategoryEntity)relatedEntity;
				this.PerformSetupSyncRelatedEntity( _category, new PropertyChangedEventHandler( OnCategoryPropertyChanged ), "Category", Relations.CategoryEntityUsingCategoryId, true, new string[] {  } );
			}
		}
		...
		{
			this.PerformDesetupSyncRelatedEntity( _supplier, new PropertyChangedEventHandler( OnSupplierPropertyChanged ), "Supplier", Relations.SupplierEntityUsingSupplierId, true, signalRelatedEntity, "Products", resetFKFields, new int[] { (int)ProductFieldIndex.SupplierId } );
			_supplier = null;
		}

		/// <summary> setups the sync logic for member _supplier</summary>
		/// <param name="relatedEntity">Instance to set as the related entity of type entityType</param>
		private void SetupSyncSupplier(IEntity2 relatedEntity)
		{
			if(_supplier!=relatedEntity)
			{
				DesetupSyncSupplier(true, true);
				_supplier = (SupplierEntity)relatedEntity;
				this.PerformSetupSyncRelatedEntity( _supplier, new PropertyChangedEventHandler( OnSupplierPropertyChanged ), "Supplier", Relations.SupplierEntityUsingSupplierId, true, new string[] {  } );
			}
		}
